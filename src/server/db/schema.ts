...
const runTagColumns = Object.fromEntries(
  tags.map((tag) => [
    `tag_${tag.code}`,
    boolean(`tag_${tag.code}`).notNull().default(false),
  ])
) as {
  [Code in TagCode as `tag_${Code}`]: HasDefault<
    NotNull<MySqlBooleanBuilderInitial<`tag_${Code}`>>
  >;
};
...
export const runs = table(
  "run",
  {
    ...
    visibility: mysqlEnum("visibility", ["public", "unlisted", "private"])
      .notNull()
      .default("private"),
  ...
  (run) => ({
    ...
    visibilityDeletedArchivedCombinedIntoRunIdIdx: index(
      "visibilityDeletedArchivedCombinedIntoRunId_idx"
    ).on(run.visibility, run.deleted, run.archived, run.combinedIntoRunId),
  })
);
...
export const runFiles = table(
  ...
  (runFile) => ({
    runIdPartNumberIdx: index("runIdPartNumber_idx").on(
      runFile.runId,
      runFile.partNumber
    ),
  })
...
export const runInteraction = table(
  ...
  (runInteraction) => ({
    runIdTypeUserIdIdx: index("runIdType_idx").on(
...
export const dataCollectionStudyParticipations = table(
  "userDataCollectionResearchParticipation",
  {
    userId: varchar("userId", { length: 255 }).notNull().primaryKey(),
    excludedSinceU18: boolean("exludedSinceU18").notNull().default(false),
    keepDataAfterStudyConducted: boolean(
...
export const dataCollectionStudyParticipationRelations = relations(
  dataCollectionStudyParticipations,
  ({ one }) => ({
    user: one(users, {
...
export const userDemographics = table(
  "userDemographic",
  {
    ...
    genderPreferNotToDisclose: boolean("gender_prefer_not_to_disclose")
      .notNull()
      .default(false),
    genderPreferToSelfDescribe: boolean("gender_prefer_to_self_describe")
      .notNull()
      .default(false),
  ...
  (userDemographic) => ({
    ...
    participantIdIdx: index("participantId_idx").on(
      userDemographic.participantId
    ),
  })
);
...
export const studyParticipantRelations = relations(
  studyParticipant,
  ({ one }) => ({
    user: one(users),
...
export const userStudyInformedConsentRelations = relations(
  userStudyInformedConsent,
  ({ one }) => ({
    participant: one(studyParticipant, {
...
export const userStudyTimeSlotRelations = relations(
  userStudyTimeSlot,
  ({ one }) => ({
    participant: one(studyParticipant, {
