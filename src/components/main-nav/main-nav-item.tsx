...
export const MenuItemContextProvider: Component<{
  ...
  return (
    <MenuItemContext.Provider value={props.value}>
      {props.children}
...
export const MenuItem: Component<MenuItemProps> = (props) => {
  ...
  const currentIsActive = () =>
    props.isActive?.({ pathname: location.pathname }) ??
    props.href === location.pathname;

  const href = () =>
    typeof props.href === "function"
      ? props.href({ pathname: location.pathname })
      : props.href;
    ...
    <>
      <Show when={props.icon}>
        <Dynamic
          ...
          class={cn("mr-1 h-4 w-4", context.iconClass)}
        />
      </Show>
