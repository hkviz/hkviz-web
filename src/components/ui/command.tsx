...
const CommandEmpty: Component<ParentProps<CommandPrimitive.CommandEmptyProps>> = (props) => {
	...
	return <CommandPrimitive.CommandEmpty class={cn('py-6 text-center text-sm', local.class)} {...others} />;
};
...
const CommandItem: Component<ParentProps<CommandPrimitive.CommandItemProps>> = (props) => {
	...
	return (
		<CommandPrimitive.CommandItem
			cmdk-item=""
			class={cn(
				'relative flex cursor-default items-center rounded-sm px-2 py-1.5 text-sm outline-hidden select-none aria-selected:bg-accent aria-selected:text-accent-foreground data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50',
				local.class,
			)}
			{...others}
		/>
...
const CommandShortcut: Component<ComponentProps<'span'>> = (props) => {
	...
	return <span class={cn('ml-auto text-xs tracking-widest text-muted-foreground', local.class)} {...others} />;
};
