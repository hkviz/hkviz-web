...
const toastVariants = cva(
	'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-(--kb-toast-swipe-end-x) data-[swipe=move]:translate-x-(--kb-toast-swipe-move-x) data-[swipe=move]:transition-none data-opened:animate-in data-closed:animate-out data-[swipe=end]:animate-out data-closed:fade-out-80 data-closed:slide-out-to-right-full data-opened:slide-in-from-top-full sm:data-opened:slide-in-from-bottom-full',
	{
		variants: {
			variant: {
	...
	return (
		<Portal>
			<ToastPrimitive.Region>
				<ToastPrimitive.List
					class={cn(
						'fixed top-0 z-100 flex max-h-screen w-full flex-col-reverse gap-2 p-4 sm:top-auto sm:right-0 sm:bottom-0 sm:flex-col md:max-w-105',
						local.class,
					)}
					{...others}
				/>
...
const ToastClose = <T extends ValidComponent = 'button'>(props: PolymorphicProps<T, ToastCloseButtonProps<T>>) => {
	...
	return (
		<ToastPrimitive.CloseButton
			class={cn(
				'absolute top-2 right-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity group-hover:opacity-100 group-[.destructive]:text-destructive-foreground group-[.error]:text-error-foreground group-[.success]:text-success-foreground group-[.warning]:text-warning-foreground focus:opacity-100 focus:ring-2 focus:outline-hidden',
				local.class,
			)}
			{...others}
		>
...
export { showToast, showToastPromise, Toast, ToastClose, ToastDescription, Toaster, ToastTitle };
