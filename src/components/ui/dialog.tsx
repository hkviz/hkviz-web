...
const DialogOverlay = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, DialogOverlayProps<T>>) => {
	...
	return (
		<DialogPrimitive.Overlay
			class={cn(
				'fixed inset-0 z-50 bg-background/80 backdrop-blur-xs data-closed:animate-out data-closed:fade-out-0 data-expanded:animate-in data-expanded:fade-in-0',
				props.class,
			)}
			{...rest}
		/>
	...
	return (
		<DialogPortal>
			...
			<DialogPrimitive.Content
				class={cn(
					'fixed top-1/2 left-1/2 z-50 grid max-h-screen w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto border bg-background p-6 shadow-lg duration-200 data-closed:animate-out data-closed:fade-out-0 data-closed:slide-out-to-left-1/2 data-closed:slide-out-to-top-[48%] data-closed:zoom-out-95 data-expanded:animate-in data-expanded:fade-in-0 data-expanded:slide-in-from-left-1/2 data-expanded:slide-in-from-top-[48%] data-expanded:zoom-in-95 sm:rounded-lg',
					props.class,
				)}
				{...rest}
			>
				{props.children}
				<DialogPrimitive.CloseButton class="absolute top-4 right-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none data-expanded:bg-accent data-expanded:text-muted-foreground">
					<svg
...
const DialogTitle = <T extends ValidComponent = 'h2'>(props: PolymorphicProps<T, DialogTitleProps<T>>) => {
	...
	return (
		<DialogPrimitive.Title class={cn('text-lg leading-none font-semibold tracking-tight', props.class)} {...rest} />
	);
};
