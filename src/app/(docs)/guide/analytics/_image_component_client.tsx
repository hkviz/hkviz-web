        ...
        <ImageAreaShadowContext.Provider value={contextValue}>
            <div
                className={
                    ...
                    (positionClassName ?? 'left-0 top-0 h-full w-full')
                }
            ></div>
            {children}
...
export function ImageArea({
    ...
    return (
        <Button
            className={cn(
                'absolute z-20 rounded-sm border-2 border-dashed border-transparent bg-transparent p-0 text-transparent no-underline drop-shadow-glow-md transition duration-200 ease-in-out hover:bg-transparent hover:bg-opacity-0 group-hover:border-white',
