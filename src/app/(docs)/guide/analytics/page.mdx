...
<SpoilerWrapper title="Show late game spoilers">
    ...
    **Godhome**:
    ...
    Analytics can also be displayed for each boss individually, by clicking on one of the bosses above the analytics.
     ...
     Similarly buildings like the Black Egg Temple and Bretta's house display the total of the rooms inside when using , but contain information about each room inside.
