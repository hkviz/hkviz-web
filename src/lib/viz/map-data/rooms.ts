...
export function scaleBounds(bounds: Pick<Bounds, 'min'|'max'>): Bounds {
	const min = {
		x: scale(bounds.min.x),
		y: scale(-bounds.max.y), // since y is inverted between svg and unity
	};

	const max = {
		x: scale(bounds.max.x),
		y: scale(-bounds.min.y),  // since y is inverted between svg and unity
	};
...
export const roomData = roomDataUnscaled.rooms.map(room => {
	...
	function roomPositionWithPadding(spriteInfo: Exclude<typeof room.roughSpriteInfo | typeof room.spriteInfo, null|undefined>) {
	...
	const color = room.mapZone === 'GODS_GLORY' ? d3.color('#f8ecd7')! :
		room.mapZone === 'WHITE_PALACE' ? d3.color('#dfe2e4')! :
