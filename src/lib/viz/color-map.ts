...
const colorMapViridisCool: RoomColorMap = {
	...
	getColorLight(value: number) {
		...
		const color = d3.interpolateCool(value);
		if (!color) return ERROR_COLOR;
		const colorMapColor = d3.hsl(color);
		return (
			colorMapColor.copy({ s: colorMapColor.s * 1.35, l: colorMapColor.l ** 0.7 * 0.75 })?.formatHex() ??
			ERROR_COLOR
		);
	},
	nameDark: 'Viridis',
	getColorDark(value: number) {
		const color = d3.interpolateViridis(value);
		if (!color) return ERROR_COLOR;
		const colorMapColor = d3.color(color);
		return colorMapColor?.formatHex() ?? ERROR_COLOR;
	},
};
...
const colorMapPlasma: RoomColorMap = {
	...
	nameDark: 'Plasma',
	getColorDark(value: number) {
		const color = d3.interpolatePlasma(value);
		if (!color) return ERROR_COLOR;
		const colorMapColor = d3.color(color);
		return colorMapColor?.formatHex() ?? ERROR_COLOR;
	},
	...
	getColorLight(value: number) {
		...
		const color = d3.interpolateInferno(value * 0.9);
		if (!color) return ERROR_COLOR;
		const colorMapColor = d3.hsl(color);
		return colorMapColor?.copy({ s: colorMapColor.s * 1.1, l: colorMapColor.l * 0.9 })?.formatHex() ?? ERROR_COLOR;
	},
};
...
const colorMapRed: RoomColorMap = {
	...
	nameDark: 'Red',
	getColorDark(value: number) {
		const color = d3.interpolatePuRd(1 - value);
		if (!color) return ERROR_COLOR;
		const colorMapColor = d3.color(color);
		return colorMapColor?.formatHex() ?? ERROR_COLOR;
	},
	getColorLight(value: number) {
		const color = d3.interpolatePuRd(value);
		if (!color) return ERROR_COLOR;
		const colorMapColor = d3.hsl(color);
		return colorMapColor?.copy({ s: colorMapColor.s * 1.1, l: 1 - colorMapColor.l })?.formatHex() ?? ERROR_COLOR;
	},
};
