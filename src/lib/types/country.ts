...
const countryByCode = new Map<CountryCode, Country>(countries.map((c) => [c.code, c]));

export function countryName(code: CountryCode): CountryName {
	return countryByCode.get(code)!.name;
}

export function countryByNameIfExists(code: string | null): Country | null {
	return !code ? null : (countryByCode.get(code as CountryCode) ?? null);
}
