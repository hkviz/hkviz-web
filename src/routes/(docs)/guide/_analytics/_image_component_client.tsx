...
export function ImageArea(props: { positionClass: string; children: JSXElement; href: string }) {
	...
	return (
		<div>
			...
			<Button
				class={cn(
					'peer absolute min-h-0 min-w-0 rounded-sm border-2 border-dashed border-black bg-transparent p-0 no-underline opacity-30 drop-shadow-glow-md transition duration-200 ease-in-out group-hover:opacity-100 hover:bg-transparent dark:border-white',
			...
			<span
				id={'tooltip' + id}
				class={
					'pointer-events-none absolute top-[.15rem] left-[50%] z-20 w-max translate-x-[-50%] rounded-md border bg-popover px-3 py-2 text-center text-sm text-popover-foreground shadow-md transition ' +
