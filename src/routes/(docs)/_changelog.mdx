...
- **Navigate map room via arrow-keys**: while on the map select rooms via the arrow keys.
- **Sub-rooms arrow keys**: the sub-rooms (like bosses in pantheons or basement/entrance in Brettas house) are now selectable via arrow keys left+right and are automatically scrolled to when a sub-room is selected.
- **Smaller map legend**: the map legend has been space-optimized to take less space away of the map.
- **First visited at clickable**: in the room-infos panel click a first-visited time to jump to it in the timeline.
