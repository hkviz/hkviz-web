import { Button, cn } from '@hkviz/components';
...
export function ImageArea(props: { positionClass: string; children: JSXElement; href: string }) {
    ...
    return (
        <div>
            ...
            <Button
                class={cn(
                    'drop-shadow-glow-md peer absolute min-h-0 min-w-0 rounded-sm border-2 border-dashed border-black bg-transparent p-0 no-underline opacity-30 transition duration-200 ease-in-out hover:bg-transparent hover:bg-opacity-0 group-hover:opacity-100 dark:border-white',
            ...
            <span
                id={'tooltip' + id}
                class={
                    'bg-popover text-popover-foreground pointer-events-none absolute left-[50%] top-[.15rem] z-20 w-max translate-x-[-50%] rounded-md border px-3 py-2 text-center text-sm shadow-md transition ' +
