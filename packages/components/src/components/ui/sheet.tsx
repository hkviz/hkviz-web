...
const SheetOverlay = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, DialogOverlayProps>) => {
...
const SheetContent = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, DialogContentProps>) => {
    const [local, others] = splitProps(props as DialogContentProps, ['position', 'class', 'children']);
    return (
        <SheetPortal position={local.position}>
            <SheetOverlay />
            <SheetPrimitive.Content class={cn(sheetVariants({ position: local.position }), local.class)} {...others}>
                {local.children}
...
const SheetTitle = <T extends ValidComponent = 'h2'>(props: PolymorphicProps<T, DialogTitleProps>) => {
    const [local, others] = splitProps(props as DialogTitleProps, ['class']);
    return <SheetPrimitive.Title class={cn('text-foreground text-lg font-semibold', local.class)} {...others} />;
};

type DialogDescriptionProps = SheetPrimitive.DialogDescriptionProps & { class?: string | undefined };

const SheetDescription = <T extends ValidComponent = 'p'>(props: PolymorphicProps<T, DialogDescriptionProps>) => {
    const [local, others] = splitProps(props as DialogDescriptionProps, ['class']);
    return <SheetPrimitive.Description class={cn('text-muted-foreground text-sm', local.class)} {...others} />;
};
